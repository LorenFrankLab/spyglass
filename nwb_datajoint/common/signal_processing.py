...
def hilbert_decomp(lfp_band_object, sampling_rate = 1):
    ...
    envelope = pynwb.ecephys.ElectricalSeries(name=eseries_name, 
    ...
    phase = pynwb.ecephys.ElectricalSeries(name=eseries_name, 
                                           data=instantaneous_phase,
                                           electrodes=lfp_band_object.electrodes, 
                                           timestamps=lfp_band_object.timestamps)

    eseries_name = "frequency"  
    frequency = pynwb.ecephys.ElectricalSeries(name=eseries_name, 
                                    data=(np.diff(instantaneous_phase) / (2.0*np.pi) * sampling_rate),
                                    electrodes=lfp_band_object.electrodes, 
                                    timestamps=lfp_band_object.timestamps)
    return envelope, phase, frequency
