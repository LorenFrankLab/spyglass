...
5. **Zero schema migration.** All v3 tables are designed in their final shape in Phase 1 (or in the phase that adds them). No `alter()` calls across phases. Concretely: Phase 1's `SortingSelection` declares two nullable typed FKs (`recording_id` → `Recording`, `concat_recording_id` → `ConcatenatedRecording`) with XOR enforced in `insert_selection()`; Phase 1's `Sorting.Unit` part table for brain-region tracing is present from day one; Phase 1's `CurationV3` columns are final. The "two paths for `SortingSelection`" formerly in Open Question #8 is collapsed to one design.
