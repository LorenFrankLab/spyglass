...
4. **Multi-day concatenation is in scope.** `SessionGroup` accepts members spanning multiple dates by default. `ConcatenatedRecording` defaults to `dredge_fast` motion correction for groups whose members span ≥2 dates and to `rigid_fast` for single-day groups. The user is not asked for an `allow_multi_day=True` opt-in; the schema makes both paths first-class.
