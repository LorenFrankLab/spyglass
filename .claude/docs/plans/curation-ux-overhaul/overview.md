...
1. **Hosted FigPack feasibility — package/protocol spike resolved; live-service gate remains.** FigPack 0.3.20 + `figpack-spike-sorting` 0.1.14 support both halves: `SortingCuration(curation=...)` serializes an initial editable state, the recursive bundle uploader includes arbitrary root sidecars, the hosted frontend loads `annotations.json`, and its edit service persists saved annotations back to that figure. Phase 3 therefore builds one seeded bundle first (including `annotations.json` + `spyglass_curation.json`) and sends that exact bundle to local or hosted delivery; `preview_import` reads the same annotations endpoint. The deterministic upload-path integration test verifies the sidecars before the mocked network boundary. No `FIGPACK_API_KEY` was available in the implementation environment, so a real hosted edit/save/read-back remains an explicit release-gate test rather than a claimed live result. `upload=False` stays the documented default until that credentialed gate passes; there was no observed protocol failure that would trigger the external-storage fallback.
