...
(it depended on phase-0). Phases 1, 1b, 1c, 2, 3a, 3b, the rest of 4a, 4b, 4c,
...
- Phases (each ships as a separable PR):
  ...
  - [phase-1b-curation-evaluation.md](phase-1b-curation-evaluation.md) — **MUST after phase-1.** Canonical curation-state evaluation and final metrics over committed merged units; keeps the phase-1 raw/merged namespace guard while adding the proper post-merge metric path.
  - [phase-1c-curation-composition-acceptance.md](phase-1c-curation-composition-acceptance.md) — parent-state child-curation composition, label inheritance, and explicit acceptance helpers for `CurationEvaluation` outputs. Split from phase-1b so final metrics can ship first.
