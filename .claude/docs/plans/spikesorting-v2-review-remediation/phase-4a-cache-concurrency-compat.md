...
14. **REL-4 — concat-backed curation query routing.** `CurationV2.resolve_restriction` routes a `{preprocessing_params_name}` restriction down the single-recording chain only (`rec_keys`), silently omitting concat-backed curations that share that recipe; the valid `{concat_recording_id, preprocessing_params_name}` pair is wrongly rejected as contradictory. Fix the routing so a `preprocessing_params_name` restriction also matches concat-backed curations (and the concat+preproc pair is accepted). **Depends on phase-0** (which decomposes `resolve_restriction` into `_curation_routing.classify_and_normalize_restriction`) — land this *after* phase-0 so the fix is a small change in the testable routing helper, not the monolith. Add a test: a `{preprocessing_params_name}` restriction returns both recording- and concat-backed curation merge ids.
