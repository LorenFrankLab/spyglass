...
2. **`Sorting` provenance columns.** Add to the `Sorting` master definition (`sorting.py:1135`, before the closing `"""`), as secondary attributes:
   ...
   Populate in `make_compute`/`make_insert` from `resolve_effective_seed(...)`, `spikeinterface.__version__`, and `importlib.metadata.version(<sorter dist>)` (NULL for `clusterless_thresholder`). Map sorter name → distribution (e.g. `mountainsort5`) with a small dict; unknown/in-process → NULL.

3. **`UnitMatch` provenance columns + `MatcherParameters` bundle params.**
   - Add to the `UnitMatch` master (`unit_matching.py:457`): `spikeinterface_version: varchar(32)`, `matcher_backend: varchar(64)`, `matcher_backend_version=null: varchar(64)`. Populate in `make_compute`/`make_insert` from the resolved registry entry's module path + `importlib.metadata.version("unitmatchpy")`.
