...
3. **Register the new module(s) for the import-boundary contract.** Add `"_curation_routing"` **and the artifact-naming module from task 1a (if extracted as a new `_artifact_naming`)** (and confirm `_curation_plan` is already present) to `_DB_FREE_SERVICE_MODULES` in `tests/spikesorting/v2/test_service_import_contracts.py:20-49`. (If task 1a folds the parser into `_curation_transforms` instead, that module is already covered.)
...
| `test_curation_routing.py::test_classify_normalizes_artifact_id_and_warning` | a `restrict_by_artifact` interval-list name normalizes to the right `artifact_detection_id` (UUID) in the plan, a raw uuid string normalizes to a `UUID`, an unresolvable name sets `unresolved_name_warning` (not raised), and `artifact_detection_id=None` is preserved for the anti-join. No `exclude/include` vocabulary. |
...
| `test_service_import_contracts.py::test_service_module_imports_without_db_layer[_curation_routing]` (and `[_artifact_naming]` if task 1a created it) | each new DB-free module cold-imports in a subprocess without pulling `spyglass.common` or any v2 schema module. |
...
- New DB-free tests exercise real branching (unknown-key strict/lenient, source classification, `artifact_detection_id` normalization + unresolved-name warning, is_merge_preview), not tautologies. (The new DB-free *unit* tests call the functions directly and need no fixtures; the **integration regression** tests reuse the existing shared fixtures rather than copy-pasting setup.) The routing helper must NOT introduce an `exclude/include` directive — it returns normalized restrictions + `artifact_detection_id` + a warning flag.
