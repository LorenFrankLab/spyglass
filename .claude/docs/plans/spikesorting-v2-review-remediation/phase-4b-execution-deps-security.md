...
3. **R32-DISP-4 — allowlist the external-whiten interception.** `_sorting_dispatch.py:504-515` intercepts **any** truthy `whiten`. Restrict the interception to the curated internal-whiten sorter set (the sorters whose schemas reject internal `whiten`); for a generic/permissive sorter with its own meaningful `whiten`, pass it through unchanged (do not silently rewrite). Document that curated MS rows use pinned external whitening.
...
5. **R34 — permissions, path confinement, trust docs.**
   ...
   - Add basename/path-confinement to NWB filename handling: in `common_nwbfile.py:107` and `mixins/analysis.py:341-348`, reject a `nwb_file_name` containing path separators / `..` / absolute paths (a `resolve().relative_to(root)` check or a basename assertion) so a crafted filename can't escape the configured root. (Defense-in-depth — accident prevention under the trusted-operator model.)
...
7. **R15 — narrow the eager v2 merge-table probe.** `spikesorting_merge.py:36-44` catches `Exception` broadly and, on any failure, permanently drops the `CurationV2` part for the process. Narrow the `except` to `ImportError`/`ModuleNotFoundError` (a genuine v0/v1-env absence) so a transient/unexpected error surfaces instead of silently disabling v2; keep the v0/v1 graceful path.
