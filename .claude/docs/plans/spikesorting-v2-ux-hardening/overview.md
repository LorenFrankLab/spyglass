...
- `src/spyglass/spikesorting/v2/pipeline.py:47` — `list_pipeline_presets()`: Phase 1 adds a sibling `describe_pipeline_presets()`; `list_pipeline_presets` is preserved unchanged.
- `src/spyglass/spikesorting/v2/pipeline.py:29-45` — `_PipelinePreset` Pydantic model (`preprocessing_params_name`, `artifact_detection_params_name`, `sorter`, `sorter_params_name`): Phase 1 extends it with optional human-facing fields (`intended_use`, `threshold_units`, `notes`) and populates them for the three built-in presets at `pipeline.py:64-83`.
...
- `src/spyglass/spikesorting/v2/pipeline.py:208-235` — the three `insert_selection` → `populate` stage blocks (recording, artifact_detection, sorting): Phase 3 instruments these; Phase 2 reads (does not insert) the same upstream tables for existence checks.
...
- A scientist can discover what each preset does (`describe_pipeline_presets()`) without reading module source.
...
- **No schema migrations.** Adding optional fields to the in-memory `_PipelinePreset` Pydantic model and adding methods/exceptions is not a DataJoint schema change; no `params_schema_version` bump, no table-`definition` edits. (Consistent with the project's pre-release schema policy.)
- **No removal of `insert_curation`.** The wrappers are thin sugar over it.

### Dependency policy

No new third-party dependencies. `pandas` (used by `describe_pipeline_presets`) is already a core Spyglass dependency; import it lazily inside the function to keep `import spyglass.spikesorting.v2` free of heavy imports, matching the existing lazy-import convention in `__init__.py:1-11`.
...
| --- | --- | --- |
| Preset catalog | `describe_pipeline_presets()` over the 3 shipping presets | Phase 5: `register_preset`, KS4 + concat presets, richer `_params/preset.py` model |
...
- `describe_pipeline_presets`, `preflight_v2_pipeline`, and the curation wrappers are new symbols — nothing depends on them yet.
