...
- **Implement `preflight_v2_pipeline()`** in `pipeline.py`, signature mirroring `run_v2_pipeline`'s inputs:
  ...
  1. `pipeline_preset_known` — `pipeline_preset in _PIPELINE_PRESETS`; on fail, list available pipeline presets and point at `describe_pipeline_presets()`. (Resolve the bundle; later checks need its param names. If this fails, short-circuit the param-row checks since their names are unknown.)
...
| `test_preflight_unknown_pipeline_preset` | Bogus pipeline preset → fail; message lists available presets / `describe_pipeline_presets()`; no DB access attempted for param rows. |
| `test_preflight_sorter_not_installed` (db) | Monkeypatch `installed_sorters()` to exclude the pipeline preset's sorter → `sorter_installed` fails with the install-path message; `clusterless_thresholder` pipeline preset still passes the sorter check. |
...
| `test_identity_payload_extraction_stable` (db) | After extracting the identity-payload builders, `recording_id`/`artifact_detection_id`/`sorting_id` for the smoke config are unchanged vs the pre-refactor values (behavior-preserving). |
