...
- **Implement `preflight_v2_pipeline()`** in `pipeline.py`, signature mirroring `run_v2_pipeline`'s inputs:
  ...
  6. `preproc_params_exist` — `PreprocessingParameters & {"preproc_params_name": bundle.preproc_params_name}`; on fail, point at `initialize_v2_defaults()`.
  7. `artifact_params_exist` — `ArtifactDetectionParameters & {"artifact_params_name": bundle.artifact_params_name}`.
  ...
  - **Warnings (non-blocking):** if the resolved `artifact_params_name` is the no-op `"none"` pass-through, append a `warnings` advisory that no artifact masking will be applied (informational, not a failure). Do **not** warn for `"default"`: the shipped default artifact params perform amplitude-threshold artifact detection, and the built-in presets legitimately use them.
  - **`expected_ids`:** for the selection stages whose inputs all exist (recording/artifact/sorting), compute the deterministic PK via `deterministic_id(kind, payload)` using the extracted payload builders, and check existence with `& pk`. Populate `expected_ids` per [shared-contracts.md](shared-contracts.md#preflight-report-schema). If a prerequisite for an ID is missing, omit that stage's entry (don't fabricate an ID from incomplete inputs). Do not include `curation_id`; Phase 3 records the actual curation key returned by `insert_curation`.
  - `ok = not errors`, where `errors = [c.message for c in checks if not c.ok]`.
