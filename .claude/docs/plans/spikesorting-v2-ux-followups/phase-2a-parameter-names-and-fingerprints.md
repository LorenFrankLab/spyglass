...
- **Describe `detect_threshold` accurately in the preset metadata.** Per SI 0.104.3 source (see [appendix.md](appendix.md)): MS4's `detect_threshold` is a **standard-deviation multiple of the ZCA-whitened signal** (default 3 ≈ 3σ) — `ms4alg.py:60` thresholds the raw whitened signal with no internal normalization, the wrapper whitens first via std-based ZCA, and post-whiten std is empirically ≈1.0. It is **not** an absolute voltage and **not** a MAD multiplier (MAD belongs to the clusterless `detect_peaks` path, `threshold_unit ∈ {uv, mad}`). Set the MS4/MS5 preset `notes`/`threshold_units` to "σ of the whitened signal (~3), not absolute voltage" — the `_PIPELINE_PRESETS` MS4/MS5 notes at [pipeline.py:680-700](../../../../src/spyglass/spikesorting/v2/pipeline.py#L680-L700) currently say "median-absolute-deviation multiplier" and should read the σ wording instead. MS4 (3) and MS5 (5.5) are the **same** whitened-σ scale (5.5 is just more conservative tuning, not a different unit).
