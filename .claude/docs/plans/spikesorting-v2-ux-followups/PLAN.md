...
(1) rename the `run_v2_pipeline` return object from "manifest" to "run summary"
everywhere user-facing, including the one public API attribute that carries the
word; (2a) correct v2's shipped "franklab" parameter blobs to the real
...
- Phases (each ships as a separable PR, in order):
  - [phase-1-run-summary-rename.md](phase-1-run-summary-rename.md) — rename "manifest" → "run summary" in prose + the `partial_manifest` API attribute; no pipeline behavior change. **Lands first** so later code uses the renamed term.
  ...
  - [phase-3-discovery-and-polish.md](phase-3-discovery-and-polish.md) — `describe_intervals()`, notebook placeholder fix, zero-unit docstring cross-refs, gated-stub note, and docs/notebook surfacing of the canonical catalog. Depends on 2a.
  - [phase-4-session-runner.md](phase-4-session-runner.md) — `run_v2_pipeline_session()` multi-sort-group batch runner with per-group outcome reporting. Depends on Phase 1 + 2a.
