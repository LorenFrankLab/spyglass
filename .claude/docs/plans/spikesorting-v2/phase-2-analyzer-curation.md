...
1. **Pydantic params validation**: `QualityMetricParameters`, `AutoCurationRules`, and `AutoCurationRules.Rule` reject bogus metric names / preset names / operators at insert; rule order is stored as part rows and is queryable by `metric_name`, `label`, and `rule_index`.
