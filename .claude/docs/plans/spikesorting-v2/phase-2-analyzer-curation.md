...
- **Implement `metric_curation.py`** per [designs.md § AnalyzerCuration](designs.md#analyzercuration-replaces-v1-metriccuration--burstpair). Specific:
  ...
  - `AnalyzerCuration` Computed with `make()` that computes additional extensions, runs `compute_quality_metrics(..., metric_params=...)`, applies label rules, runs `compute_merge_unit_groups`, writes three NWB tables (`quality_metrics`, `merge_suggestions`, `proposed_labels`) via `AnalysisNwbfile().build()`. **When the label rules flag zero units (a normal outcome — e.g. clean units under the default `nn_noise_overlap > 0.1` rule), `proposed_labels` must OMIT the ragged label column** rather than write an all-empty list-of-lists, which fails pynwb dtype inference (#1625). Mirror the CurationV2 curated-units writer (`_units_nwb.py` adds the column only when ≥1 unit is labeled; `get_labels` reads via `.get("curation_label", [])`). Per [Empty / NaN / Boundary Invariant 4](shared-contracts.md#empty--nan--boundary-invariants).
...
- **SI 0.104 metric-name + `isi_violation` behavior** (verified against the installed SI 0.104.3 source — see [appendix.md § SpikeInterface 0.104.3 source-verified findings](appendix.md#spikeinterface-01043--source-verified-findings-read-first)):
...
- **Add a static units-QC overview plot** (notebook-facing, sibling to `plot_sort_group_geometry`). The per-unit `describe_units` table (the pre-Phase-2 sort-time helper) answers "what units did I get?"; this answers "do they look reasonable as a population?" -- the at-a-glance QC view that closes the "is this sort good?" evaluation gulf, and which no Spyglass version (v0/v1/v2) currently provides in any form (v1's FigURL surface shows metrics only as a per-unit lookup table, never as distributions or a depth scatter). Keep it **static matplotlib**, DB-backed, no kachery/cloud dependency, so it lands with the metrics in this phase rather than waiting on the interactive FigPack work (Phase 5). It reads `AnalyzerCuration`'s computed `quality_metrics` (via `get_metrics`) plus the `unit_locations` extension (already computed here for auto-merge), and renders two views: (a) **metric-distribution histograms** -- one small-multiple per metric (`snr`, `isi_violation`, `presence_ratio`, `amplitude_cutoff`, `firing_rate`, ...); (b) an **amplitude-vs-depth / firing-rate-vs-depth scatter** with each unit placed at its peak-channel probe position, reusing the probe-coordinate machinery from `plot_sort_group_geometry` (`Probe.Electrode.rel_x/rel_y`, NOT the zero/NULL `Electrode.x/y/z`), colored/sized by a chosen metric. Accept an optional `ax`/axes and return the matplotlib axes (the `plot_sort_group_geometry` signature contract). Handle the empty/degenerate cases the rest of the phase already guards: a **zero-unit** sort returns an empty-but-labeled figure (it must not raise), and the **NaN metrics** that `compute_quality_metrics` legitimately produces on low-spike units (sanitized for serialization elsewhere) are dropped from the histograms / annotated on the scatter rather than silently distorting an axis. Name the symbol for what it shows (e.g. `plot_units_qc`), never for the phase. This is the static complement to the interactive FigPack curation app (Phase 5): FigPack gives the per-unit sortingview deep-dive for manual label/merge; this gives the population QC overview. Mirrors AIND's split (static matplotlib QC figures in `aind-ephys-processing-qc`, separate from the interactive sortingview curation app).
...
- **Implement the v2 clusterless `spike_location` decoding mark** (closes review item **F3** in [REVIEW-REPORT.md](REVIEW-REPORT.md) and the dead-code gap in `decoding/v1/waveform_features.py`). The v2 `SortingAnalyzer` dispatch added in Phase 1b serves only `amplitude` and `full_waveform` for v2 (`CurationV2`) sources; `spike_location` is currently rejected with an explicit `NotImplementedError` ([src/spyglass/decoding/v1/waveform_features.py](../../../../src/spyglass/decoding/v1/waveform_features.py)). `spike_location` is an optional clusterless mark (the default clusterless mark is `amplitude`), so this is a feature-completeness/parity item, not a decoding blocker. Implement it on the SortingAnalyzer path:
  - Compute the `spike_locations` extension on the v2 analyzer (`depend_on == ['templates']`; `templates` is already computed at sort time per [appendix.md § SortingAnalyzer extension dependencies](appendix.md#sortinganalyzer-extension-dependencies)) and surface per-unit spike locations through the existing `_AnalyzerWaveformAccessor` adapter, so `spike_location` resolves for v2 sources the same way `amplitude` / `full_waveform` do. Use SI 0.104's analyzer/extension API (`SortingAnalyzer.compute("spike_locations", ...)`), NOT the removed `WaveformExtractor` form.
...
- **Parity test against v1 MetricCuration baseline**. New test `test_v2_analyzer_curation_vs_v1` (slow, integration): on the Phase 0 baseline-captured sort, load the legacy-environment v1 `MetricCuration` pickle and compute v2 `AnalyzerCuration` under SI 0.104. Do not invoke active v1 MetricCuration in the SI 0.104 test unless Phase 0c explicitly ported that surface. Compare the per-unit `snr`, `isi_violation`, `firing_rate`, `num_spikes` columns. Tolerances:
  ...
  - `isi_violation`: exact — but **only if v2 computes Spyglass's `count/(n_spikes−1)` fraction** (see the SI metric-name note above), not SI's `isi_violations_ratio`. The violation count is deterministic given sorted spike times; the ratio is a different (model-estimator) number and would not match v1.
...
- **Documentation update**:
  - Update [docs/src/Features/SpikeSortingV2.md](../../../../docs/src/Features/SpikeSortingV2.md) (created Phase 1) with a Quality Metrics section, including the static `plot_units_qc` overview (metric histograms + amplitude/firing-rate-vs-depth scatter) as the population QC view.
...
11. **Zero labels on a non-empty sort (#1625)**: a clean-units fixture under the default `nn_noise_overlap > 0.1` rule labels no unit; `AnalyzerCuration.make()` saves cleanly — `proposed_labels` omits the ragged label column (not an all-empty list-of-lists), `get_labels` returns empty, and there is no hdmf "Cannot infer dtype of empty list" crash. Distinct from #5: this sort HAS units; only the labels are empty.
...
12. **Units-QC overview plot** (slow): `plot_units_qc` renders against a SortingAnalyzer-backed sort under the `Agg` backend -- one histogram axis per requested metric and one scatter point per unit at its probe position; a **zero-unit** sort returns an empty-but-labeled figure without raising, and a low-spike unit's **NaN** metrics are dropped from the histogram rather than crashing or distorting it.
