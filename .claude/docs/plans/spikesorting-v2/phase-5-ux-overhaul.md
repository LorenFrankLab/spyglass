...
- Phase 5a is complete: the FigPack spike-sorting API, display/publish, and edited-curation round trip are verified (offline) and recorded in [figpack-runtime.md](../../../../tests/spikesorting/v2/resolver/figpack-runtime.md); the FigPack contract stubs in this file, `designs.md`, and `appendix.md` are replaced with the observed contract. Phase 5b implements against that verified contract.
- Extend the current single-session `pipeline.py` runner with full presets,
  auto-curation, concat routing, FigPack routing, and separate
  `run_v2_unit_match()`. Preserve the already-shipped `require_units=False`
  zero-unit behavior.
- Implement preset validation and optional FigPack dependency gates.
- Replace the current import-safe `figpack_curation.py` placeholder, update/remove
  its placeholder assertions in `tests/spikesorting/v2/test_legacy_stub_imports.py`,
  then implement `FigPackCurationSelection`, `FigPackCuration`, URI generation,
...
- **FigPack contract — verified (feasibility spike complete).** The FigPack spike-sorting API, view construction, display/publish, and edited-curation round trip are verified and recorded in [figpack-runtime.md](../../../../tests/spikesorting/v2/resolver/figpack-runtime.md); the contract stubs in this file, `designs.md`, and `appendix.md` are replaced with the observed contract. Phase 5b may implement the FigPack DataJoint tables against that contract.
...
- **Verify FigPack outside DataJoint first.** Use the isolated v2 curation environment and one existing v2 `SortingAnalyzer` fixture. Deliverables:
  ...
  6. The FigPack contract stubs in this file, `designs.md`, and `appendix.md` are replaced with the observed contract, including the exact helper shapes that `FigPackCuration.make()` / `build_curation_view()` and `FigPackCuration.fetch_curation_from_uri()` wrap (recorded in [figpack-runtime.md](../../../../tests/spikesorting/v2/resolver/figpack-runtime.md)).
...
- **Phase 5a is complete — no FigPack contract stubs remain under `.claude/docs/plans/spikesorting-v2/`; Phase 5b may proceed.**
...
- **EXTEND `pipeline.py`** (Phase 1 shipped a minimal version with 3 presets covering recording → artifact → sorting → initial curation). Phase 5 adds the missing stages and broadens the preset set per [designs.md § `run_v2_pipeline()` Orchestrator](designs.md#run_v2_pipeline-orchestrator):
  ...
  - **Expand `_PIPELINE_PRESETS`** (defined in `_pipeline_presets.py`, built from `_recipe_catalog.py` `pipeline_preset_specs()`) — the dated franklab presets already shipped in Phase 1/2a:
    - `franklab_tetrode_hippocampus_30khz_ms5_2026_06` (MountainSort5) and the `*_ms4_2026_06` family: `franklab_tetrode_hippocampus_30khz_ms4_2026_06`, `franklab_probe_{hippocampus,cortex}_{30,20}khz_ms4_2026_06` (MountainSort4)
    - `franklab_clusterless_2026_06` (clusterless thresholder) and `franklab_neuropixels_ks4_2026_06` (Kilosort4)
    - **new in this phase**: a same-day concat preset (uses SessionGroup + ConcatenatedRecording for same-day chronic; sets `motion_correction_params_name`) — this one does NOT exist yet.
  - Built-in preset names follow the dated `franklab_{probe_or_modality}_{region}_{rate}_{sorter}_{date}` pattern (e.g. `franklab_tetrode_hippocampus_30khz_ms5_2026_06`). External labs should use the same dated `{lab}_..._{date}` pattern.
...
- **Implement `figpack_curation.py`** per [designs.md § FigPackCuration](designs.md#figpackcuration):
  ...
  - `make()` uses the verified FigPack API. Publishes/uploads the view, stores returned URI. Load the analyzer via `Sorting().get_analyzer(...)` and **ensure the curation-view extensions** (`spike_amplitudes`/`correlograms`/`unit_locations`/`template_similarity`) on top of v2's base set (`random_spikes`/`noise_levels`/`templates`/`waveforms`) via the existing `Sorting.add_extensions`/`ensure_extensions` surface. **Minimal-attach view build:** let SpikeInterface compose the whole summary (`sw.plot_sorting_summary(analyzer, curation=False, backend="figpack", generate_url=False, display=False).view`) and **attach only** `ssv.SortingCuration(default_label_options=...)` as a sibling — do NOT reimplement SI's layout, and do **NOT** call `plot_sorting_summary(curation=True)` (released SI passes `label_choices=` while `figpack-spike-sorting==0.1.14` expects `default_label_options=`, raising `TypeError`; add a compatibility probe so the attach shim can be dropped once upstream aligns). **First 5b step:** smoke `build_curation_view` against one real populated v2 analyzer (Docker/MySQL v2 env) — 5a verified the contract on a standalone SpikeInterface analyzer per owner decision, not against `get_analyzer()`.
...
- **Extend the canonical v2 notebook**: update
  ...
  5. Preflight and run `run_v2_pipeline(..., pipeline_preset="franklab_tetrode_hippocampus_30khz_ms5_2026_06")` (1-2 cells).
...
- **Documentation overhaul**:
  ...
  - Update the existing `docs/src/Features/SpikeSortingV2_Migration.md` page with a "Choosing v1 or v2" section instead of adding a second overlapping decision page. TL;DR: v2 is recommended for new work under the SI 0.104 environment; active v1 runtime workflows require the legacy SI 0.99 environment unless Phase 0c explicitly ported that surface. Existing v1 sorts stay queryable through v1 where possible.
  - In `docs/src/Features/SpikeSortingV2_Migration.md`, include the import path explicitly: external or ground-truth NWB Units still use the existing `ImportedSpikeSorting` workflow and appear in `SpikeSortingOutput.ImportedSpikeSorting`; they are not reinserted as `CurationV2` rows.
...
- **Source documentation-density pass (readability follow-up).** The v2 source is ~46% documentation (≈36% docstring lines + ≈10% `#` comment lines, measured across [`src/spyglass/spikesorting/v2/`](../../../../src/spyglass/spikesorting/v2/)). That density was the right investment during the v1→v2 port, but it now hurts scannability: several functions wrap a handful of logic lines in multi-paragraph inline rationale (re-identify the worst offenders in [`recording.py`](../../../../src/spyglass/spikesorting/v2/recording.py) and the split `_recording_*`/`_sorting_*` modules at implementation time — the specific hotspots and line numbers have shifted with the module splits). Trim for signal without losing the standalone "why". This is a comments/docstrings-only pass — **no logic changes**.
  ...
  - **Scope guard.** Source comments/docstrings only. No logic edits, and no `definition`-string column-comment changes; this readability pass is not a schema-correction task (see [DOCSTRING-AUDIT.md](DOCSTRING-AUDIT.md) "Excluded from the fix pass"). The density pass diff must show only comment/docstring lines.
...
- **No unplanned schema changes to existing v2 tables.** Phase 5 only adds the planned tables (`FigPackCurationSelection`, `FigPackCuration`) and new preset registrations (`_PipelinePreset` entries in `_pipeline_presets.py` / `_recipe_catalog.py`, NOT a `_params/preset.py`). Any Phase 1–4 table-definition change must be an explicit, reviewed pre-production schema correction, not incidental UX work.
...
6. **FigPack feasibility-gated** (slow, integration, optional): the FigPack feasibility spike is complete and recorded in `figpack-runtime.md` (verified contract, no stubs remaining); with the `spikesorting-v2-curation` extra installed, `FigPackCuration.populate(key)` returns a non-empty URI or verified local artifact reference; zero-unit curation publishes an empty view or raises a clear zero-unit error (`ZeroUnitAnalyzerError` / `ZeroUnitSortError`, or a new FigPack-specific exception — there is no `EmptySortingError` in `exceptions.py`), never a missing-column `KeyError`; published labels round-trip via `fetch_curation_from_uri`.
...
PY
test -f tests/spikesorting/v2/resolver/figpack-runtime.md  # FigPack contract verified
...
- FigPack feasibility was verified before implementation began, `tests/spikesorting/v2/resolver/figpack-runtime.md` records the working API and round trip, and the FigPack contract stubs are resolved (verified contract recorded). If FigPack had proved unusable, the project owner would have been escalated and no degraded fallback shipped.
- All docs tasks landed: `docs/src/Features/SpikeSortingV2.md` banner, README snippet, and an updated `docs/src/Features/SpikeSortingV2_Migration.md` with the v1/v2 decision guidance.
...
- Sanity: `git diff` against any Phase 1–4 table `definition` strings is empty unless the PR explicitly documents a reviewed pre-production schema correction.
