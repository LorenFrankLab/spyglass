...
- **No schema changes to existing v2 tables.** Per the zero-migration policy, Phase 5 only ADDS new tables (`FigPackCurationSelection`, `FigPackCuration`) and new preset registrations (`_PipelinePreset` entries in `_pipeline_presets.py` / `_recipe_catalog.py`, NOT a `_params/preset.py`). Any change to Phase 1–4 table definitions is forbidden.
