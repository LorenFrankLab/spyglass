...
**No new tables in Phase 3.** The schema declared in Phase 1 (`SessionGroup`, `MotionCorrectionParameters`, `ConcatenatedRecordingSelection`, `ConcatenatedRecording`) becomes populate-runnable. Phase 3 is mostly method-body work plus the non-schema selection-identity helper update for concat-backed `sorting_id` determinism.
...
erDiagram
    ...
    SortingSelection ||--o{ SortingSelection_ArtifactDetectionSource : "optional single-session artifact part"
    ...
    ArtifactDetection ||--o{ SortingSelection_ArtifactDetectionSource : "FK (for single-session sorts only)"
...
- **Concat sorts skip artifact masking.** `SortingSelection.insert_selection()` rejects `ConcatenatedRecordingSource` together with any `ArtifactDetectionSource` part. Cross-recording artifact masking is out of scope.
