...
- Use the dedicated conda environment for v2 development: `conda env create -f environments/environment_spikesorting_v2.yml` (env name `spyglass_spikesorting_v2`, Python 3.11, SpikeInterface 0.104). Do not install SpikeInterface 0.104, UnitMatchPy, MEArec, FigPack, or resolver-test dependencies into the shared `spyglass` conda environment (it is pinned to SpikeInterface 0.99).
...
- Phase 0c and any checkpoint that verifies third-party APIs records the exact versions used (`python --version`, `spikeinterface.__version__`, `pip freeze`, and relevant import/installed-sorter probes) in a checked-in artifact under `tests/spikesorting/v2/resolver/`. PR descriptions may summarize it, but the branch keeps the artifact as the durable evidence.
...
3. **Spike at recording boundary is valid.** The WRITE path runs `sic.remove_excess_spikes(sorting, recording)` in `Sorting.make()`: a boundary spike is either kept (within the recording) or dropped with a logged warning — never a "spikes exceeding recording duration" raise (matching v1). The READBACK path (`_signal_math._spike_times_to_frames`, the v2 reimplementation of v1's `spike_times_to_valid_samples`, reconstructing frames from persisted ABSOLUTE spike times) CLAMPS a `searchsorted` frame that lands one past the end back to the last sample when the spike is within ~2 sample periods of the end (floating-point rounding of the final spike's absolute time) rather than dropping it — dropping would desync per-spike features (the `UnitWaveformFeatures` n_feat==n_spikes check). A spike genuinely beyond that tolerance RAISES an alignment/units error (which the write-path drop should have prevented).
4. **`CurationV2.insert_curation` accepts `labels=None` (the default), equivalent to `{}`.** This is a deliberate v1-parity relaxation (`v1/curation.py:44-49`): `None` and `{}` both produce an unlabeled curation (the shipped code normalizes `None` to `{}`). **When no unit is labeled, the ragged `curation_label` column is OMITTED entirely** — writing an all-empty list-of-lists makes pynwb fail dtype inference ("Cannot infer dtype of empty list", the [#1625](https://github.com/LorenFrankLab/spyglass/issues/1625) crash). The column is written only when ≥1 unit carries a label, and readers tolerate its absence via `nwb_sorting.get("curation_label", [])`. `CurationV2.UnitLabel` has zero rows when `labels` is `None`/`{}`. The **same omit-when-all-empty rule applies to any ragged label column the Phase 2 `AnalyzerCuration` `proposed_labels` write produces** — auto-curation flagging zero units (e.g. clean units under the default `nn_noise_overlap > 0.1` rule) is a normal outcome that must save cleanly, never an opaque hdmf dtype error.
