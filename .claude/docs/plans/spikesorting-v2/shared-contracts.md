...
4. **`CurationV2.insert_curation` accepts `labels=None` (the default), equivalent to `{}`.** This is a deliberate v1-parity relaxation (`v1/curation.py:44-49`): `None` and `{}` both produce an unlabeled curation (the shipped code normalizes `None` to `{}`). The NWB writer still materializes the `curation_label` column (empty list per unlabeled unit); `CurationV2.UnitLabel` has zero rows when `labels` is `None`/`{}`. (This corrects an earlier draft of this invariant that wrongly required an explicit argument — the code and `test_*` lock the `None` default for v1 parity.)
