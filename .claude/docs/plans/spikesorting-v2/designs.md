...
- **Analyzer extensions + minimal-attach view build.** `make()`/`build_curation_view()` load the analyzer via `Sorting().get_analyzer(...)`, whose base set is only `random_spikes`/`noise_levels`/`templates`/`waveforms`. The curation view additionally needs `spike_amplitudes`/`correlograms`/`unit_locations`/`template_similarity`, so the builder ensures them on top via the existing `Sorting.add_extensions`/`ensure_extensions` surface (not assume they are present). Build the view by **letting SpikeInterface compose the whole summary** — `sw.plot_sorting_summary(analyzer, curation=False, backend="figpack", generate_url=False, display=False).view` — and **attaching only** `ssv.SortingCuration(default_label_options=...)` as a sibling layout item. Do **not** reimplement SI's layout, and do **not** call `plot_sorting_summary(curation=True)`: released SI passes `label_choices=` while `figpack-spike-sorting==0.1.14` expects `default_label_options=` (unresolved upstream → `TypeError`). Add a compatibility probe so the attach shim can be dropped once upstream aligns. See [figpack-runtime.md](../../../../tests/spikesorting/v2/resolver/figpack-runtime.md).
