...
- **Bump dependencies for the v2 runtime environment.**
  ...
  - Add optional dependency group `spikesorting-v2-matching = ["UnitMatchPy>=3.2.6,<3.2.8", "mat73"]` only after resolver testing confirms it does not force an incompatible NumPy downgrade in the supported Python range and the UnitMatchPy import path is usable or clearly guarded. (The `<3.2.8` cap is deliberate: 3.2.8+/3.3.x reactively flipped to `numpy<2.0` per upstream #134; the 3.2.6/3.2.7 line is the last to declare `numpy>=2.0,<3.0`.)
...
| `test_optional_matching_extra_resolution` | The `spikesorting-v2-matching` extra includes both `UnitMatchPy>=3.2.6,<3.2.8` and `mat73`, resolves without NumPy incompatibility, and import guards produce a clear message if UnitMatchPy hits the `_tkinter` import path. |
