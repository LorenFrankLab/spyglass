...
- **Declare the concat-scaffolding tables in Phase 1** (zero-migration policy: every FK target must exist in the phase that introduces the FK). This includes:
  ...
  - `ConcatenatedRecording` (Computed) — DECLARED with full `definition` including the `MemberBoundary` part table, but `make()` raises `NotImplementedError("ConcatenatedRecording.make() is not implemented yet")`. The schema is final from Phase 1 so `SortingSelection` can FK it; the populate is deferred.
