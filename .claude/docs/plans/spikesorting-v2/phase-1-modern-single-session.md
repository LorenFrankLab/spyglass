...
3. `pyproject.toml` SI pin bumped to `>=0.104,<0.105`; `mountainsort5>=0.5` added; optional `spikesorting-v2-matching` extra added.
...
- **Implement `sorting.py`** — `SorterParameters`, `SortingSelection`, `Sorting` per [designs.md § SorterParameters + SortingSelection + Sorting](designs.md#sorterparameters--sortingselection--sorting). Specific points:
  - **`SortingSelection` schema is FINAL in Phase 1** per the zero-migration policy. Columns: `sorting_id` PK; `-> [nullable] Recording` (adds `recording_id`); `-> [nullable] ConcatenatedRecording` (adds `concat_recording_id`); `-> SorterParameters`; `-> [nullable] ArtifactDetection` (real FK, NOT a loose `artifact_id=NULL: uuid` — see designs.md and the round-5 #1437 fix). **XOR enforced via the three-layer defense from [shared-contracts.md § Nullable XOR Foreign-Key Pattern](shared-contracts.md#nullable-xor-foreign-key-pattern)**: Layer 1 = `_validate_xor` called inside `insert_selection()`; Layer 2 = `_validate_xor` re-run at the start of `Sorting.make()` to catch direct `dj.Manual.insert1()` bypasses; Layer 3 = `test_no_xor_violations_in_production` integrity test. Phase 1's `insert_selection` rejects `concat_recording_id` with `NotImplementedError("Concat path requires Phase 3")`; the validator gate is what changes in Phase 3, not the schema.
...
| `test_v2_empty_sorting_phase1` | A zero-unit sorting populates cleanly: `Sorting.populate()` succeeds with `n_units=0`, `Sorting.Unit` has zero rows, `CurationV2.insert_curation(sort_key, labels={})` succeeds, and `CurationV2.Unit` / `CurationV2.UnitLabel` have zero rows. |
| `test_curation_v2_auto_registers_in_merge` | After `CurationV2.insert_curation(...)`, `SpikeSortingOutput.CurationV2 & key` has one row. |
| `test_curation_v2_insert_atomic_on_merge_register_failure` | Monkey-patch `SpikeSortingOutput.insert` to raise during the merge-part step inside `insert_curation()`. Assert that NO row exists in any of: `CurationV2 & sorting_key`, `CurationV2.Unit & sorting_key`, `CurationV2.UnitLabel & sorting_key`, `SpikeSortingOutput.CurationV2 & sorting_key`. Also assert the staged curated-units analysis file was removed or is absent from the untracked-file scan. Same test pattern with monkey-patched `CurationV2.UnitLabel.insert` (Step 4 failure) — also no partial DB state and no orphan file. |
| `test_curation_v2_label_enum_enforced` | `CurationV2.insert_curation(labels={1: ["typo_label"]})` raises `ValueError`. `labels={1: ["accept", "mua"]}` succeeds and inserts two `CurationV2.UnitLabel` rows. |
| `test_spike_sorting_output_routes_v2_in_get_restricted_merge_ids` | `SpikeSortingOutput.get_restricted_merge_ids(key, sources=['v2'])` returns the v2-sourced merge_ids only; `sources=['v1', 'v2']` returns both. |
| `test_spike_sorting_output_v2_restriction_surface` | Restrictions by `nwb_file_name`, `team_name`, `sort_group_id`, `interval_list_name`, `preproc_params_name`, `recording_id`, `artifact_id`, `sorter`, `sorter_params_name`, `sorting_id`, and `curation_id` all return the expected v2 merge_id and do not match unrelated imported/v1 rows. |
| `test_spike_sorting_output_source_class_dict_has_curation_v2` | `from spyglass.spikesorting.spikesorting_merge import source_class_dict; assert source_class_dict["CurationV2"]` is the v2 CurationV2 class. The dict is module-level (see [spikesorting_merge.py:26](../../../../src/spyglass/spikesorting/spikesorting_merge.py#L26)), NOT a class attribute on `SpikeSortingOutput`. |
| `test_spike_sorting_output_get_recording_works_for_v2` (slow) | After inserting a v2 sort into the merge, `SpikeSortingOutput.get_recording(merge_key)` returns a BaseRecording (does NOT KeyError on dispatch). |
...
| `test_sorting_make_rechecks_xor_against_bypass_insert` | Directly insert a SortingSelection row with both recording_id and concat_recording_id NULL via `dj.Manual.insert1()` (bypassing the helper). Calling `Sorting.populate(key)` raises with a message naming the offending PK and pointing at `_validate_xor`. Regression test for Layer 2 of the three-layer XOR defense. |
| `test_artifact_detection_make_rechecks_xor_against_bypass_insert` | Same pattern for `ArtifactDetectionSelection` / `ArtifactDetection.populate()`: bypass insert → populate raises. |
| `test_no_xor_violations_in_production` | Layer 3 integrity test in `tests/spikesorting/v2/test_integrity.py`. Queries both `SortingSelection` and `ArtifactDetectionSelection` for `both-NULL` and `both-set` rows; asserts result sets are empty. Runs in the v2 CI job (lightweight; safe to run on every push). |
...
| `test_sorter_params_generic_available_sorter` | Pick an installed SI sorter without a dedicated v2 schema; inserting an explicit custom params row validates through `GenericSorterParamsSchema`. A bogus sorter name raises with the available sorter list. |
| `test_sorting_mountainsort4_run` (slow, integration) | Run v2 with MS4 against `minirec`; assert `Sorting & key` row produced; `n_units > 0`. |
...
| `test_sorting_get_unit_brain_regions_returns_dataframe` (slow) | Returned DataFrame has columns `unit_id`, `electrode_id`, `region_name`, `peak_amplitude_uV`, `n_spikes`, `region_resolution`. For single-session sorts `region_resolution == 'single_session'` on every row. |
| `test_sorting_get_unit_brain_regions_raises_on_concat_by_default` | Build a `SortingSelection` row whose `concat_recording_id` is set (skip the Phase-3 NotImplementedError gate via a Phase 3 fixture or a Phase 1 mock). `Sorting.get_unit_brain_regions(key)` raises `ConcatBrainRegionAmbiguousError`; the message names `TrackedUnit.get_unit_brain_regions` and `allow_anchor_member=True`. |
| `test_sorting_get_unit_brain_regions_allow_anchor_member` | Same fixture but call with `allow_anchor_member=True`. Returns rows with `region_resolution == 'anchor_member'` on every row; values match the first SessionGroup.Member's Electrode → BrainRegion join. |
| `test_curation_v2_get_unit_brain_regions_concat_guard` | Same default-raise / explicit-opt-in semantics on `CurationV2.get_unit_brain_regions`. |
| `test_curation_v2_unit_part_populated` | `CurationV2.insert_curation(sort_key, labels={}, ...)` populates `CurationV2.Unit` with one row per surviving unit (after merges applied); merged unit inherits peak channel from highest-amplitude contributor. |
| `test_curation_v2_unit_filter_by_label` | `CurationV2.get_unit_brain_regions(key, include_labels=["accept"])` restricts via `CurationV2.UnitLabel` and returns units carrying the `accept` label, including multi-labeled units. |
| `test_curation_v2_matchable_unit_ids_excludes_any_bad_label` | Units labeled `reject`, `noise`, or `artifact` are excluded even if also labeled `accept`; unlabeled, `accept`, and `mua` units are included. |
| `test_curation_v2_get_sort_group_info_returns_all_electrodes` | Test fixture with a multi-region sort group (synthetic); `get_sort_group_info(key).fetch("region_name")` returns ALL regions, NOT just one (regression fix vs v1's `fetch(limit=1)`). |
| `test_spike_sorting_output_get_unit_brain_regions_for_v2` (slow) | `SpikeSortingOutput.get_unit_brain_regions(merge_key)` works on v2 merge_ids and returns the same DataFrame as `CurationV2.get_unit_brain_regions(curation_key)`. |
| `test_sorted_spikes_group_works_with_v2` (slow) | Insert a `SortedSpikesGroup` row using a v2-sourced merge_id; assert `SortedSpikesGroup.get_spike_times(key)` returns sane numpy arrays. |
| `test_v2_minirec_plumbing` (slow) | Run v2 with `clusterless_thresholder` against `minirec`. Asserts the *pipeline plumbing* works end-to-end (no exception; merge_id minted; `SortGroupV2`, `Recording`, `Sorting`, `CurationV2` rows produced) — **does NOT assert anything about sort correctness** (`minirec` is too short to contain real spikes). This is the regression guard for "did v2 ingestion / populate chain break". |
| `test_v2_mearec_polymer_ground_truth` (slow, integration) | Run v2 with `clusterless_thresholder` against `mearec_polymer_60s.nwb` (the Phase-0-generated ground-truth fixture). Use `spikeinterface.comparison.compare_sorter_to_ground_truth(gt_sorting, v2_sorting)` against the planted Units table. Assert per-unit `accuracy ≥ 0.8` for at least 4 of 6 planted units. **This is the real correctness test.** |
| `test_v2_mearec_neuropixels_ground_truth` (slow, integration) | Same as above against `mearec_neuropixels_60s.nwb` with MountainSort 5. Assert per-unit `accuracy ≥ 0.7` for at least 15 of 20 planted units (MS5 stochasticity wider than clusterless thresholder). |
| `test_v2_mearec_brain_region_correct` (slow) | On `mearec_polymer_60s.nwb` with planted `brain_region_map = {0: "CA1", 1: "CA3"}`, run v2 sort and assert `Sorting.get_unit_brain_regions(key)` returns regions that match the planted soma → peak-channel → group mapping. Directly tests the unit→region tracing invariant. |
| `test_v2_real_data_v1_parity` (slow, integration, requires env var) | If `SPIKESORTING_V2_REAL_NWB_PATH` is set, run v2 + v1 against that NWB and compare per-unit spike times. Tolerance: `clusterless_thresholder` exact within ±1 sample; MS4/MS5 within ±50% unit count + ±30% median FR. Skipped with explicit message if env var unset. No minirec-based parity assertion is used. |
