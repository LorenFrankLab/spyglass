...
- `SPIKESORTING_V2_REAL_NWB_PATH` env-var gated: v1↔v2 parity per tolerances (`clusterless_thresholder` ±1 sample; MS4/MS5 ±50% unit count + ±30% median FR). Skipped if env var unset. MS4/MS5 comparisons are aggregate sanity checks only because MS4 is not deterministic; they are not spike-time or unit-ID equivalence gates. The test uses the isolated database/write directories; `SPYGLASS_ALLOW_PRODUCTION_SMOKE=1` permits read-only production metadata lookup only.
