...
9. **Forward-compat schema (zero-migration)**: `SortingSelection.heading.attributes` includes nullable `concat_recording_id`; `concat_recording_id`-non-NULL `insert_selection` raises a clear "not implemented yet" error until concat support lands; `ConcatenatedRecording.populate()` raises `NotImplementedError`; `ConcatenatedRecording.MemberBoundary` exists; `Sorting.object_id` and `CurationV2.object_id` are `varchar(72)` and use `object_id` (not `units_object_id`) per the merge convention.
