...
- **Do not add direct Pydantic or Zarr pins in Phase 0.** Pydantic is required by the v2 `_params/` models, but it enters Spyglass transitively through the SpikeInterface 0.104 prerequisite PR. Zarr is also a SpikeInterface runtime dependency (`zarr>=2.18,<3` with `numcodecs<0.16.0`) and is not a v2 design dependency because v2 persists SortingAnalyzers as `binary_folder`, not Zarr. Only add an explicit Spyglass pin later if the SI upgrade PR exposes a concrete resolver/runtime issue that cannot be handled by SpikeInterface's own dependency metadata.
