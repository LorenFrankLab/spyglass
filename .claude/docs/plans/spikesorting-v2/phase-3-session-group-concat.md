...
- **Implement Phase-3 method bodies on the Phase-1-declared `session_group.py` schema:**
  ...
  - `ConcatenatedRecording` Computed with `make()` that:
    ...
    8. `split_sorting_by_session(sorting, key) -> dict[tuple[str, int, str], BaseSorting]` back-maps spike times; keys are the hashable member identity `(nwb_file_name, sort_group_id, interval_list_name)` (the full member dict is not hashable; `sort_group_id` is in the key so same-nwb/interval members on distinct sort groups do not collide).
