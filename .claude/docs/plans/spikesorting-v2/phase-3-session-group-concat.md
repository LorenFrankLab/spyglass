...
- **Implement back-mapping helper** `ConcatenatedRecording.split_sorting_by_session(sorting, key) -> dict[tuple[str, str], si.BaseSorting]`. Binding behavior: fetch the `ConcatenatedRecordingSelection` row first; load members ordered by `member_index`; use `ConcatenatedRecording.MemberBoundary` rows to slice spike trains into each member's local sample frame; return hashable keys `(nwb_file_name, interval_list_name)`; preserve unit IDs across members.
