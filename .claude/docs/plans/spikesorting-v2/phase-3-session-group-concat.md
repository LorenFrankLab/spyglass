...
9. **Phase 1 SortingSelection schema unchanged**: `SortingSelection.heading.attributes` and source part definitions are byte-identical to Phase 1 (no migration); concat source requests are accepted now; source-part integrity is still enforced; concat + non-NULL `artifact_id` still rejected.
