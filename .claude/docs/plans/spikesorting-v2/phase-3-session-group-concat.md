...
Implements the concatenate-and-sort workflow on top of the SessionGroup / ConcatenatedRecording schema that Phase 1 already declared. `SessionGroup` is the grouping table; `ConcatenatedRecording` is a materialized motion-corrected, unwhitened concat cache, not a lightweight group alias. Phase 3 is mostly method-body work: fills in `ConcatenatedRecording.make()`, lifts `SortingSelection.insert_selection`'s `ConcatenatedRecordingSource` rejection, adds `SessionGroup.create_group`'s `allow_multi_day` validation logic, and updates the non-schema sorting-identity helper path for concat-backed selections. **Default scope is same-day**; multi-day requires `allow_multi_day=True` and an explicit (non-`auto`) motion-correction preset. For cross-day analyses the recommended path is Phase 4 sort-then-match (UnitMatch), not concat — concat-across-days is experimental.

## Executor Checklist

- Keep table definitions unchanged from the **post-review-fixes baseline** (NOT the original Phase 1 shape — review-fixes already corrected `SortingSelection` to use the optional `ArtifactDetectionSource` part, `SortGroupV2.reference_mode`, etc.); this checkpoint is mostly method-body work plus the non-schema source-identity helper updates needed for concat-backed `sorting_id` determinism.
...
- Extend the sorting identity/preflight helpers so concat-backed selections fold `concat_recording_id` into `sorting_id` exactly as single-recording selections fold in `recording_id` today.
...
- **No table-definition changes.** Phase 3 modifies Phase-1-declared code in `session_group.py` and `sorting.py`: implement `ConcatenatedRecording.make()`, lift `SortingSelection.insert_selection()`'s concat rejection, remove the concat-source `Sorting.key_source` antijoin, add `Sorting.make()` recording-resolution dispatch, and add `SessionGroup.create_group()` multi-day validation. It also updates the selection identity/preflight helper path for concat-backed `sorting_id` derivation. Definition strings stay unchanged.
- **Implement Phase-3 method bodies on the Phase-1-declared `session_group.py` schema:**
  ...
  - `ConcatenatedRecording` Computed with `make()` that:
    ...
    2. **Reuses cached NWB-resident `Recording` artifacts per member** rather than re-preprocessing from raw NWB. All members must share the `PreprocessingParameters` row on the `ConcatenatedRecordingSelection`; the selection-time check enforces this by requiring a populated per-member `Recording` row for that same `preprocessing_params_name`. For each member, fetch the matching `RecordingSelection` PK; load the BaseRecording via `Recording.get_recording(rec_key)`. **No nested `Recording.populate()` — that is a DataJoint anti-pattern.** `make()` defensively re-checks and raises `MissingRecordingForConcatError` with the offending member key if the precondition was bypassed. This avoids the silent-divergence risk of preprocessing twice AND keeps populate behavior easy to reason about. See [designs.md § SessionGroup + ConcatenatedRecording](designs.md#sessiongroup--concatenatedrecording) make() body.
    3. `concatenate_recordings(rec_list)` → mono-segment recording view before the concat cache is written.
    4. Applies `correct_motion(rec, preset=..., **preset_kwargs)` if preset != "none"; `preset_kwargs` comes from the Phase-1-declared `MotionCorrectionParameters.params` blob. Phase 0c must have recorded the pinned SI 0.104 `correct_motion` signature before Phase 1 froze this schema. The Phase 1 Pydantic schema rejects kwargs that change the return type or write untracked side artifacts (`folder`, `output_motion`, `output_motion_info`, `overwrite`, or their pinned-API equivalents) because Phase 3 deliberately does not persist queryable motion trajectories. The persisted artifact is the corrected, unwhitened recording; motion estimates require a future opt-in side table, not a mutation of `ConcatenatedRecording`.
    5. Writes the motion-corrected, unwhitened recording as an `ElectricalSeries` inside an `AnalysisNwbfile` (NWB-resident — same backend as Phase 0 picked for `Recording`; see [shared-contracts.md § Recording Cache Format](shared-contracts.md#recording-cache-format)). **Do not call `PreprocessingParamsSchema.to_post_motion_dict()` here.** Whitening remains a sorter/analyzer concern: MS4/MS5 use the `SorterParameters.params["whiten"]` external-whitening path, KS4 whitens internally, display analyzers stay unwhitened, and metric analyzers opt into whitening through `AnalyzerWaveformParameters`. This cache may be large: it intentionally duplicates the per-member `Recording` caches after motion correction so the sorter receives one continuous recording. Use narrow `IntervalList` members when sorting a subset of a day.
    6. Records cumulative sample boundaries in `ConcatenatedRecording.MemberBoundary` rows for back-mapping spike times.
    7. `get_recording(key)` reads the cache.
    8. `split_sorting_by_session(sorting, key) -> dict[tuple[str, int, str, str], BaseSorting]` back-maps spike times; keys are the hashable member identity `(nwb_file_name, sort_group_id, interval_list_name, team_name)` (the full member dict is not hashable; `sort_group_id` + `team_name` are in the key so same-nwb/interval members on distinct sort groups or teams do not collide).

- **Lift the concat-source restriction in `SortingSelection.insert_selection()`**. No table-definition changes: Phase 1 declared both source parts (`RecordingSource` and `ConcatenatedRecordingSource`) in the post-review-fixes baseline shape. Phase 3's `sorting.py` modifications:
  - Remove the `raise NotImplementedError("Concatenated recording sorting is not implemented yet")` guard that Phase 1 installed for `ConcatenatedRecordingSource` insert requests.
  - The source-part validation (exactly one INPUT source — `RecordingSource` XOR `ConcatenatedRecordingSource`; `ArtifactDetectionSource` is not an input source and is not counted) stays unchanged from the post-review-fixes baseline.
  - Add a method-body guard that rejects a `ConcatenatedRecordingSource` request that also asks for an artifact: concat rows must have **no `ArtifactDetectionSource` row** (post-review-fixes; was "non-NULL `artifact_detection_id`"). Artifact detection for concat recordings is a later feature because Phase 3 reuses per-member `Recording` artifacts and does not define concat-wide artifact intervals.
  - Extend `_selection_identity.sorting_identity_payload(...)` and the `_selection_plan.build_sorting_selection_plan(...)` preflight path to accept a concat source identity. For `RecordingSource`, the payload continues to include `recording_id`; for `ConcatenatedRecordingSource`, the payload must include `source_kind="concat"` and `concat_recording_id` and must not fabricate or omit a `recording_id`. `SortingSelectionPlan` must also carry enough structured source information for `insert_selection()` and `Sorting.make()` to insert/load the correct source part (for example `source_kind` plus either a `recording_source_row` or `concat_source_row`, rather than a recording-only row). A given `concat_recording_id` + sorter + sorter params + artifact association state yields a stable `sorting_id`; changing the concat source changes the `sorting_id`. This is the contract that lets the same concat-backed `sorting_id` resolve to the same recording window.

- **Update `Sorting.make()`** to dispatch through a small recording-resolution helper. Binding behavior: a row with `RecordingSource` loads `Recording.get_recording()`, a row with `ConcatenatedRecordingSource` loads `ConcatenatedRecording.get_recording()`, and a row with no source or multiple sources raises the same source-bypass error as Phase 1. This is a Phase 3 *method-body* change to existing `Sorting.make()`; not a schema change. Also add deterministic parent anchoring for analysis NWB writes: single-recording rows anchor to `RecordingSelection.nwb_file_name`; concat rows anchor to the first `SessionGroup.Member.nwb_file_name`. The complete multi-session provenance remains queryable through `SortingSelection -> SortingSelection.ConcatenatedRecordingSource -> ConcatenatedRecordingSelection -> SessionGroup.Member`; do not query `RecordingSelection` with a concat-only selection row.

- **Implement back-mapping helper** `ConcatenatedRecording.split_sorting_by_session(sorting, key) -> dict[tuple[str, int, str, str], si.BaseSorting]`. Binding behavior: fetch the `ConcatenatedRecordingSelection` row first; load members ordered by `member_index`; use `ConcatenatedRecording.MemberBoundary` rows to slice spike trains into each member's local sample frame; return hashable keys `(nwb_file_name, sort_group_id, interval_list_name, team_name)`; preserve unit IDs across members.
...
- **No concat-wide artifact detection.** `SortingSelection.insert_selection()` rejects a `ConcatenatedRecordingSource` request that also supplies an artifact: concat rows must have **no `ArtifactDetectionSource` row** (post-review-fixes; was "non-NULL `artifact_detection_id`"). Artifact intervals remain single-recording or shared-recording-group inputs until a later phase defines concat artifact semantics.
- **No table-definition changes to `SortingSelection`.** Under the pre-production schema policy, this phase intentionally leaves the `definition` string untouched **relative to the post-review-fixes baseline** (the optional `ArtifactDetectionSource` part already replaced the nullable `artifact_detection_id` FK in review-fixes). The concat identity work is a helper/preflight change, not a schema edit.
...
5. **MotionCorrectionParameters validation**: bogus presets raise at insert; a `MotionCorrectionParameters` row whose params blob contains `preset="auto"` succeeds on single-day groups and raises on multi-day populate.
6. **ConcatenatedRecording shape** (slow): row has `analysis_file_name`, `electrical_series_path`, `object_id`; `n_channels` matches members; `total_duration_s` = sum(member durations); `MemberBoundary` rows are cumulative integer sample counts ordered by `member_index`; the persisted concat recording is motion-corrected but unwhitened, so display analyzers and sorter/analyzer whitening remain governed by their own parameter rows.
7. **No nested populate / preselection** (slow): `ConcatenatedRecordingSelection.insert_selection` raises `MissingRecordingForConcatError` if any per-member `Recording` matching the shared `preprocessing_params_name` is missing; `ConcatenatedRecording.make()` consumes cached `Recording` artifacts (monkey-patch `Recording.populate` to raise — it must NOT be called); `make()` defensively re-checks if `insert_selection` was bypassed.
...
9. **SortingSelection schema unchanged from the post-review-fixes baseline**: `SortingSelection.heading.attributes` and source/association part definitions (including the optional `ArtifactDetectionSource` part that review-fixes added) are byte-identical to the **post-review-fixes baseline**; concat source requests are accepted now; input-source integrity (exactly one of `RecordingSource`/`ConcatenatedRecordingSource`, `ArtifactDetectionSource` excluded) is still enforced; a concat request that also supplies an artifact (would create an `ArtifactDetectionSource` row) is still rejected; `SpikeSortingOutput.get_restricted_merge_ids(sources=['v2'], key={"concat_recording_id": ...})` resolves through `SortingSelection.ConcatenatedRecordingSource` and can combine that restriction with sorter, curation, and session-group fields. An `artifact_detection_id` restriction resolves through the optional `ArtifactDetectionSource` part (not a master column).
9a. **Concat sorting identity determinism**: two insert/preflight requests for the same `concat_recording_id`, sorter, sorter parameters, and artifact association state resolve to the same `sorting_id`; changing only `concat_recording_id` changes `sorting_id`; single-recording requests keep their existing `recording_id`-based identity; the returned preflight plan carries concat source information through to source-part insertion/loading instead of using recording-only fields. A regression test should call the identity/preflight helper directly so this contract fails before any expensive populate work.
...
**Motion-correction scientific gate** (slow, integration): on `mearec_polymer_128ch_drift_120s.nwb`, compare `preset="rigid_fast"` against `preset="none"` via `compare_sorter_to_ground_truth`. The gate is aggregate, not monotonic per unit: corrected output should improve the median or mean accuracy for the drift-affected planted units beyond a calibrated tolerance, while allowing some individual units to stay unchanged or degrade. Record the baseline distribution in the fixture manifest and fail only on aggregate non-improvement or large unexplained regressions. Multi-day smoke test gated on `--run-chronic`.
...
pytest tests/spikesorting/v2/test_session_group_concat.py -q   # new behavioral tests this phase adds
pytest tests/spikesorting/v2/test_sorting_contracts.py -q       # SortingSelection schema stability (the old test_single_session_pipeline.py monolith was split into single_session/)
...
- **No table-definition changes to `SortingSelection`.** Git diff against `src/spyglass/spikesorting/v2/sorting.py` shows the `definition` string is byte-identical to the **post-review-fixes baseline schema** (which already includes the optional `ArtifactDetectionSource` part). Method-body changes and the `_selection_identity.py` / `_selection_plan.py` helper changes are reviewed separately; the schema-stability test passes against that baseline.
