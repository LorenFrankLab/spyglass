...
- **Lift the concat-source restriction in `SortingSelection.insert_selection()`**. NO schema changes — Phase 1 declared both source parts (`RecordingSource` and `ConcatenatedRecordingSource`) in their final shape (see [shared-contracts.md § Zero-Migration Schema Forward-Compatibility](shared-contracts.md#zero-migration-schema-forward-compatibility)). Phase 3's only modification to `sorting.py`:
  ...
  - The source-part validation (exactly one of `RecordingSource` / `ConcatenatedRecordingSource` must exist) stays unchanged from Phase 1.
  - Add a method-body guard that rejects `ConcatenatedRecordingSource` together with non-NULL `artifact_id`. Artifact detection for concat recordings is a later feature because Phase 3 reuses per-member `Recording` artifacts and does not define concat-wide artifact intervals.
...
- **No concat-wide artifact detection.** `SortingSelection.insert_selection()` rejects `ConcatenatedRecordingSource` with non-NULL `artifact_id`; artifact intervals remain single-recording or shared-recording-group inputs until a later phase defines concat artifact semantics.
...
9. **Phase 1 SortingSelection schema unchanged**: `SortingSelection.heading.attributes` and source part definitions are byte-identical to Phase 1 (no migration); concat source requests are accepted now; source-part integrity is still enforced; concat + non-NULL `artifact_id` still rejected; `SpikeSortingOutput.get_restricted_merge_ids(sources=['v2'], key={"concat_recording_id": ...})` resolves through `SortingSelection.ConcatenatedRecordingSource` and can combine that restriction with sorter, curation, and session-group fields.
