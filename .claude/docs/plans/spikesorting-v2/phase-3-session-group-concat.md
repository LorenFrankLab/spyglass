...
Implements the concatenate-and-sort workflow on top of the SessionGroup / ConcatenatedRecording schema that Phase 1 already declared. `SessionGroup` is the grouping table; `ConcatenatedRecording` is a materialized sorter-ready cache, not a lightweight group alias. Phase 3 is method-body-only: fills in `ConcatenatedRecording.make()`, lifts `SortingSelection.insert_selection`'s `ConcatenatedRecordingSource` rejection, and adds `SessionGroup.create_group`'s `allow_multi_day` validation logic. **Default scope is same-day**; multi-day requires `allow_multi_day=True` and an explicit (non-`auto`) motion-correction preset. For cross-day analyses the recommended path is Phase 4 sort-then-match (UnitMatch), not concat — concat-across-days is experimental.

## Executor Checklist

- Keep schemas unchanged from the **post-review-fixes baseline** (NOT the original Phase 1 shape — review-fixes already corrected `SortingSelection` to use the optional `ArtifactSource` part, `SortGroupV2.reference_mode`, etc.); this checkpoint is method-body-only against that corrected baseline.
...
- Lift the concat-source gate in `SortingSelection.insert_selection()` while keeping the source-part guard and the concat-artifact guard (concat rows must have **no `ArtifactSource` row**).
...
- **Implement Phase-3 method bodies on the Phase-1-declared `session_group.py` schema:**
  ...
  - `ConcatenatedRecording` Computed with `make()` that:
    ...
    4. Applies `correct_motion(rec, preset=..., **preset_kwargs)` if preset != "none"; `preset_kwargs` comes from the Phase-1-declared `MotionCorrectionParameters.params` blob. Phase 0c must have recorded the pinned SI 0.104 `correct_motion` signature before Phase 1 froze this schema. The Phase 1 Pydantic schema rejects kwargs that change the return type or write untracked side artifacts (`folder`, `output_motion`, `output_motion_info`, or their pinned-API equivalents) because Phase 3 deliberately does not persist queryable motion trajectories. The persisted artifact is the corrected sorter-ready recording; motion estimates require a future opt-in side table, not a mutation of `ConcatenatedRecording`.
...
- **Lift the concat-source restriction in `SortingSelection.insert_selection()`**. NO schema changes — Phase 1 declared both source parts (`RecordingSource` and `ConcatenatedRecordingSource`) in their final shape (see [shared-contracts.md § Zero-Migration Schema Forward-Compatibility](shared-contracts.md#zero-migration-schema-forward-compatibility)). Phase 3's only modification to `sorting.py`:
  ...
  - The source-part validation (exactly one INPUT source — `RecordingSource` XOR `ConcatenatedRecordingSource`; `ArtifactSource` is not an input source and is not counted) stays unchanged from the post-review-fixes baseline.
  - Add a method-body guard that rejects a `ConcatenatedRecordingSource` request that also asks for an artifact: concat rows must have **no `ArtifactSource` row** (post-review-fixes; was "non-NULL `artifact_id`"). Artifact detection for concat recordings is a later feature because Phase 3 reuses per-member `Recording` artifacts and does not define concat-wide artifact intervals.
...
- **No concat-wide artifact detection.** `SortingSelection.insert_selection()` rejects a `ConcatenatedRecordingSource` request that also supplies an artifact: concat rows must have **no `ArtifactSource` row** (post-review-fixes; was "non-NULL `artifact_id`"). Artifact intervals remain single-recording or shared-recording-group inputs until a later phase defines concat artifact semantics.
- **No schema changes to `SortingSelection`.** Per the zero-migration policy, this phase only modifies `Sorting.make()` and `SortingSelection.insert_selection()` method bodies; the `definition` string is untouched **relative to the post-review-fixes baseline** (the optional `ArtifactSource` part already replaced the nullable `artifact_id` FK in review-fixes).
...
9. **SortingSelection schema unchanged from the post-review-fixes baseline**: `SortingSelection.heading.attributes` and source/association part definitions (including the optional `ArtifactSource` part that review-fixes added) are byte-identical to the **post-review-fixes baseline** (no migration); concat source requests are accepted now; input-source integrity (exactly one of `RecordingSource`/`ConcatenatedRecordingSource`, `ArtifactSource` excluded) is still enforced; a concat request that also supplies an artifact (would create an `ArtifactSource` row) is still rejected; `SpikeSortingOutput.get_restricted_merge_ids(sources=['v2'], key={"concat_recording_id": ...})` resolves through `SortingSelection.ConcatenatedRecordingSource` and can combine that restriction with sorter, curation, and session-group fields. An `artifact_id` restriction resolves through the optional `ArtifactSource` part (not a master column).
...
- **No schema changes to `SortingSelection`.** Git diff against `src/spyglass/spikesorting/v2/sorting.py` shows changes ONLY inside method bodies — the `definition` string is byte-identical to the **post-review-fixes baseline schema** (which already includes the optional `ArtifactSource` part). The schema-stability test passes against that baseline.
