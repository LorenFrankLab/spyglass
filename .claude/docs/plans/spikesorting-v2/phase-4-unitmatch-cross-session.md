...
- **Walk the actual UnitMatchPy API** end-to-end against that analyzer. Document:
  ...
  - Whether v2 can produce UnitMatchPy's required split-half average waveforms from existing `SortingAnalyzer` extensions alone. If not, document the wrapper-owned trace extraction from `CurationV2.get_recording(key)` (the curation-pinned recording — there is no `Sorting.get_recording`; `get_recording` lives on `CurationV2`, `Recording`, and now `ConcatenatedRecording`, though 4a feeds the matcher single-session non-concat curated sortings) needed to create the two cross-validation halves. This is acceptable only if the matcher still consumes the prepared bundle and never receives raw NWB paths or Spyglass table keys.
...
- The MEArec-based **polymer** validation test (`test_v2_unitmatch_polymer_mearec_ground_truth`) passes its AUC > 0.85 criterion — this is the gate. If it fails, Phase 4 does not ship; the implementer escalates rather than relaxing the threshold. **Current status: local/manual validation.** The gate is verified locally (AUC > 0.85) and wired into the CI matching-extra env, but it *skips* in CI until the two-session polymer fixtures (`mearec_polymer_128ch_2sessions_{s1,s2}.nwb`) are uploaded and their URLs set in `tests/spikesorting/v2/fixtures/_fetch.py` (then add them to `SPYGLASS_V2_REQUIRE_FIXTURES` to make it hard). Uploading those fixtures is the remaining step to make CI enforcement automatic.
