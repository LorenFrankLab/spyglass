...
| `Electrode` (common_ephys.py:73, Imported) | `-> ElectrodeGroup`, `electrode_id: int` | `-> [nullable] Probe.Electrode`, **`-> BrainRegion` at common_ephys.py:79 (NON-NULL)**, `bad_channel: enum("True", "False")` | Round-5 fix confirmed: brain region is NON-null on Electrode. `bad_channel` is enum string, NOT int — v2 helpers filtering Spyglass `Electrode` must use `== "True"`, not `== 1`. |
