...
| v2 artifact intervals named `f"artifact_detection_{artifact_detection_id}"` via `insert1`; v1 used bare-UUID + `skip_duplicates=True` | **v1 unsafe defaults**: silent overwrite path. **No scientific output change.** | Not a sort-output difference; passes are unaffected. |
