...
- **Phase 5 owns the verify-current-failure tests** that pin TODAY's behavior so a future regression to silent-drift-consumption is caught:
  ...
  - **Test 2 — `_rebuild_nwb_artifact` reaches hashing, then raises checksum.** Directly call `(Recording & key)._rebuild_nwb_artifact(key)` (NOT via `get_recording`). **Implementation note (ground truth, 2026-06-01):** the original wording here assumed the direct rebuild "runs to completion (no exception inside the rebuild)" and only `get_recording`'s reader rejects the file. That is contradicted by the actual call chain: `_rebuild_nwb_artifact` → `_compute_recording_artifact` → `_write_nwb_artifact` finishes the write, then calls `_hash_nwb_recording` to record the rebuilt `cache_hash` — and `_hash_nwb_recording` reads the file through the **checksum-validated** `get_abs_path`, which rejects the rebuilt-but-not-byte-identical file. So the rebuild raises a checksum `DataJointError` from *inside its own hashing step*. The test therefore pins: (a) `_hash_nwb_recording` IS reached (monkeypatch a call counter, assert `>= 1`) — proving the rebuild machinery runs up to verification — and (b) the call raises a checksum `DataJointError` (asserting the raising path per the A18 skills directive). This still cleanly separates "the rebuild machinery runs up to hashing" from "the rebuild is non-functional vs the external-store checksum today"; the latter is what main-epic recompute fixes. When recompute lands, this test flips to the deferred happy path below.
...
| `test_rebuild_nwb_artifact_reaches_hash_then_raises_checksum` (slow) | A18 Test 2: directly invoke `_rebuild_nwb_artifact`; assert `_hash_nwb_recording` is reached (monkeypatch counter `>= 1`) AND the call raises a checksum `DataJointError`. (Ground-truth correction of the original "runs to completion" wording — the rebuild's own hash step reads via the checksum-validated `get_abs_path` and rejects the rebuilt file.) |
