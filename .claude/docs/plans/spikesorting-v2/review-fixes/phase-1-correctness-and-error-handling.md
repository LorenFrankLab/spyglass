...
- **The file-replace + DB-hash-update CANNOT be made jointly atomic by a DataJoint transaction alone** (the transaction guards only the DB write; `os.replace` is a separate filesystem op). Because `get_recording` returns an existing canonical file WITHOUT re-hashing it ([recording.py:1022]), any file/DB inconsistency at the canonical path is read back silently. The accept-drift path (`repair()` / `allow_drift=True`) MUST therefore leave file and row hash consistent under partial failure — specify ONE of:
  - **Update-DB-then-replace with delete-on-failure:** stage the new file at a temp path; commit the `cache_hash` update transaction; on commit success `os.replace` temp→canonical; if the `os.replace` fails after commit, **delete the canonical file** so the next `get_recording` re-enters rebuild and fails closed against the (now-new) stored hash — never returns a stale file under a new hash.
  - **Backup-and-restore:** copy the current canonical file to a backup; `os.replace` new→canonical; update `cache_hash`; if the DB update raises, restore the backup so the file again matches the OLD stored hash. Remove the backup only after both succeed.
  Either way: a crash between the two ops must NOT end with a canonical file whose bytes disagree with the row's `cache_hash`. Add a test that injects a DB-update failure after the file replace and asserts the next `get_recording` does not return mismatched content (it rebuilds + raises, or reads the restored last-good file).
- Add a `Recording.repair()` classmethod that consciously recomputes + atomically rewrites the cache and updates `cache_hash` using the chosen partial-failure-safe ordering above (the sanctioned drift-acceptance entry point; `get_recording(..., allow_drift=True)` delegates to it).
...
| `test_recording_repair_partial_failure_consistent` (slow) | C2: with a DB `cache_hash` update injected to fail AFTER the file replace, the canonical path never ends up with bytes that disagree with the stored hash — next `get_recording` rebuilds+raises or reads the restored last-good file. |
...
| `test_get_sorting_zero_units` (slow) | C5 (sorting loader): `Sorting.get_sorting(key)` on a zero-unit sort behaves per the documented choice (empty `NumpySorting` OR `ZeroUnitSortError`) — guarded before `NwbSortingExtractor(...)`; `CurationV2.get_sorting` inherits it. |
