...
- **Three precise outcomes (the previous "log + proceed" was ambiguous — it would either leave NO file or re-create the poison cache):**
  ...
  - **hash mismatches, `allow_drift=True`** → this means "accept the new content as canonical": `os.replace` temp → canonical **AND update the stored `cache_hash` to the rebuilt hash in the same transaction**, then log. Promoting the file WITHOUT updating the row hash is forbidden — that just recreates the drift on the next read. (`allow_drift=True` is therefore the same operation as `Recording.repair()`; implement one in terms of the other.)
- Add a `Recording.repair()` classmethod that consciously recomputes + atomically rewrites the cache and updates `cache_hash` (the sanctioned drift-acceptance entry point; `get_recording(..., allow_drift=True)` delegates to it).
