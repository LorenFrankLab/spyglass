...
- **`make_compute` Mode A cleanup** at [sorting.py:614-626](../../../../../src/spyglass/spikesorting/v2/sorting.py#L614-L626). Monkeypatch `_write_units_nwb` to raise after `_build_analyzer` succeeded. Assert (a) the analyzer folder is gone, (b) the original exception is re-raised, (c) the row was never inserted.
