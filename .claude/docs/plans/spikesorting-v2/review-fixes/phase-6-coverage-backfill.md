...
- **`get_unit_brain_regions` concat-source `ConcatBrainRegionAmbiguousError`** at [sorting.py:976-989](../../../../../src/spyglass/spikesorting/v2/sorting.py#L976-L989). Insert a concat-source sort row via the raw bypass (since `insert_selection` rejects concat today), call `get_unit_brain_regions` with `allow_anchor_member=False`, assert the exception. Then call with `allow_anchor_member=True` and assert the return is the DataFrame produced by `unit_brain_region_df(self.Unit & key, resolution)` ([sorting.py:989](../../../../../src/spyglass/spikesorting/v2/sorting.py#L989)) carrying `region_resolution == "anchor_member"` (set at [:986](../../../../../src/spyglass/spikesorting/v2/sorting.py#L986)). The return is NOT a `SourceResolution` dataclass — that type sits inside `make_fetch`'s dispatch, not on this accessor's return surface.
...
- **NOT INCLUDED — `SessionGroup.Member.LabTeam` consistency.** An earlier draft of A31 listed "Member.LabTeam matches the master's session_group_owner" as an invariant test. Source verification (`session_group.py:58-67`) shows the part-table FKs to LabTeam and to master are independent — there is no insert-time check that they agree, and the audit did NOT surface this as a bug. Adding a test that asserts a not-yet-implemented validation belongs in a behavioral phase (Phase 4), not a test-only backfill. **Deliberately deferred:** if the team decides LabTeam consistency should be enforced, file it as a new Phase 4 task (BUG, "Member.insert validates lab_team == session_group_owner") and add the corresponding regression test to A31 in the same PR. If the team prefers the current freedom (a member from one team in a group owned by another), drop the row from the next plan revision.
