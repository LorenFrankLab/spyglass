...
- [src/spyglass/spikesorting/v2/recording.py:571-651](../../../../../src/spyglass/spikesorting/v2/recording.py#L571-L651) ✓ — `PreprocessingParameters` (declaration at :571; column default at :582; `_DEFAULT_CONTENTS` rows incl. `default_franklab` at :594, `default_neuropixels` at :610, `"no_filter"` at :621; `insert_default` at :647). T5 lands here.
...
- Update every reader that does `sel_row.get("artifact_id")` to use the new `resolve_artifact` accessor / `ArtifactSource` join. **Full reader inventory** (re-grep with `grep -n 'artifact_id' src/spyglass/spikesorting/v2/sorting.py` before editing — drift is the recurring failure mode):
  - `make_fetch` `obs_intervals` derivation at [sorting.py:499](../../../../../src/spyglass/spikesorting/v2/sorting.py#L499) and the upstream fetch at [:509](../../../../../src/spyglass/spikesorting/v2/sorting.py#L509)
  - `make_compute` artifact-mask gate at [sorting.py:558](../../../../../src/spyglass/spikesorting/v2/sorting.py#L558)
  - `_rebuild_analyzer_folder` artifact reader at [sorting.py:878](../../../../../src/spyglass/spikesorting/v2/sorting.py#L878) and the corresponding fetch at [:894](../../../../../src/spyglass/spikesorting/v2/sorting.py#L894) — the audit reviewer flagged this site explicitly because analyzer rebuilds for artifact-backed sorts will break if the master FK is dropped without updating this path.
  - The `insert_selection` find-existing branch at [sorting.py:289-299](../../../../../src/spyglass/spikesorting/v2/sorting.py#L289-L299) — the `null_artifact_filter` text branch collapses with T1 (drop both the conditional and the `IS NULL` SQL fragment; "no artifact pass" = "no `ArtifactSource` row").
  - `Sorting.get_analyzer` ([sorting.py:821-853](../../../../../src/spyglass/spikesorting/v2/sorting.py#L821-L853)) does NOT directly read `artifact_id` but delegates to `_rebuild_analyzer_folder` on missing folder; once that path is migrated, `get_analyzer` does not need changes.
  - Curation paths: `CurationV2`'s helpers that mirror `Sorting`'s `obs_intervals` plumbing — grep `'artifact_id'` across `curation.py` before editing.
- Validation: add a Phase 6 cross-reference test that asserts `_rebuild_analyzer_folder` succeeds for an artifact-backed sort AFTER the migration (delete the analyzer folder, call `Sorting.get_analyzer(key)` for an artifact-backed sort, assert the rebuild does not crash). Without this test the T1 migration could ship green against existing tests but break analyzer rebuild on the next production sort.
...
- **Tri-part fetch/compute plumbing (required — the sentinel int flows through the whole dispatch, not just the table):**
  - `RecordingFetched` NamedTuple ([recording.py:732](../../../../../src/spyglass/spikesorting/v2/recording.py#L732)) ✓ currently carries `ref_channel_id: int`. Replace with `reference_mode: str` + `reference_electrode_id: int | None` (two fields, positional order matters — `make_compute` unpacks positionally). Sibling `RecordingComputed` is at [recording.py:751](../../../../../src/spyglass/spikesorting/v2/recording.py#L751) — keep its layout in sync.
  - `Recording.make_fetch` (populates `RecordingFetched`) — read the two new `SortGroupV2` columns instead of the single sentinel.
  - `Recording.make_compute` / `_compute_recording_artifact` ([recording.py:772 region](../../../../../src/spyglass/spikesorting/v2/recording.py#L772)) — accept the two fields and pass them to BOTH `_restrict_recording` and `_apply_pre_motion_preprocessing`.
  - **`_restrict_recording` ([recording.py:1340-1392](../../../../../src/spyglass/spikesorting/v2/recording.py#L1340-L1392)) ✓ also consumes the reference — do not miss it.** It takes `ref_channel_id: int` and, per its docstring ([recording.py:1376-1383](../../../../../src/spyglass/spikesorting/v2/recording.py#L1376-L1383)), **includes the specific reference electrode in the channel slice** so `common_reference` can subtract it (then it's dropped after referencing). Replace its `ref_channel_id` param with `reference_mode` + `reference_electrode_id`: include the extra channel in the slice only when `reference_mode == 'specific'`. If this path is missed, "specific" mode silently omits the reference electrode from the slice (or keeps sentinel `>= 0` logic), so referencing is wrong.
  - `_apply_pre_motion_preprocessing` ([recording.py:1725 region](../../../../../src/spyglass/spikesorting/v2/recording.py#L1725)) ✓ — switch on `reference_mode` (`'none'` → skip, `'global_median'` → global median CMR, `'specific'` → reference to `reference_electrode_id`) instead of `== -1 / == -2 / >= 0`.
...
- **Old code removed:** the sentinel-int arithmetic in BOTH `_apply_pre_motion_preprocessing` and `_restrict_recording`, AND the `ref_channel_id: int` field on `RecordingFetched`. No parallel sentinel path left behind (grep for `ref_channel_id` to confirm zero remaining uses).
...
- Change the shipped `"no_filter"` preset ([recording.py:621](../../../../../src/spyglass/spikesorting/v2/recording.py#L621)) ✓ to `bandpass_filter=None` instead of `freq_min=1.0, freq_max=14999.0`.
