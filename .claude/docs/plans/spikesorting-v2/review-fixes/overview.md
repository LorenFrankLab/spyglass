...
| C2 | HIGH | BUG | `recording.py:1077-1085,1022` | Cache-hash mismatch is `logger.warning` then returns the drifted file; worse, the drifted file is already on disk so the next `get_recording` (which only rebuilds when absent) returns it silently. Raise `RecordingCacheDriftError` by default (`allow_drift=True` opt-in) AND delete/quarantine the drifted file (atomic temp-write, promote only on hash match — shared with C7). + `Recording.repair()`. | 1 |
| C3 | HIGH | BUG | `recording.py:1478-1486` | Non-monotonic timestamps silently rewritten; no provenance. Add `timestamps_adjusted` + `n_adjusted_samples` columns (safe: v2 unreleased) + gate on a param flag. | 1 |
...
| C7 | HIGH | BUG | `recording.py:1760-1773` | `_write_nwb_artifact` unconditional `unlink()` on rebuild-failure destroys the only good cache copy. Add `existing_analysis_file_name is None` guard, or temp-write + `os.replace`. | 1 |
...
| C3 column additions to `Recording`. | Safe now (v2 unreleased, "final from introduction" not yet binding); land before any external consumer depends on the table. |
