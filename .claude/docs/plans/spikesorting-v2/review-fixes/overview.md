...
| A11 | LOW | DRIFT | `sorting.py:457-474` ✓ | `Sorting.make` has no `key_source` filter on `ConcatenatedRecordingSource`; populate picks up concat rows and prints `NotImplementedError` per row. Antijoin `key_source`. | 4 |
