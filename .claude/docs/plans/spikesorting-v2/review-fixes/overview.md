...
| T5 | HIGH | RESTRUCTURE | `recording.py:621` ✓ ; `_params/preprocessing.py:8-23` ✓ | `"no_filter"` preset at recording.py:621 uses `freq_min=1.0,freq_max=14999.0` (wide-band, not disabled). Make `bandpass_filter: BandpassFilterParams \| None`; None = skip. T5/T6 share a single `params_schema_version=2→3` bump (column default at recording.py:582). | 2 |
...
| V3 | HIGH | DEFERRED → Phase 6 A31 | `session_group.py` invariants | Original audit framing was "pin `NotImplementedError` gates + table-level validation." Per [decision 4](#settled-design-decisions), no `pytest.raises(NotImplementedError)` stub tests; behavioral / schema-invariant coverage lives in Phase 6 A31 (Member uniqueness, LabTeam FK, `ConcatenatedRecording` column-shape, `MotionCorrectionParameters.insert1` Pydantic validation, `initialize_v2_defaults` shipping motion presets). Phase 3 V3 collapses to a cross-reference. | 3 → 6 |
...
| Q2 | MED | VERIFY-GREP NO-OP | `curation.py:1138-1143` ✓ ; `sorting.py:977` ✓ | Audit's "`TrackedUnit` dangling reference" premise already resolved — `grep -rn "TrackedUnit" src/spyglass/ tests/spikesorting/` returns empty; concat-error messages now say "cross-session unit matching is not available in this build." Verify with the grep once; reopen if it ever returns a hit. | 3 |
...
| A18 | HIGH | TEST (verify-current-failure) | `recording.py:1114-1117,1138-1179,1923` ✓ | `Recording.get_recording` rebuild integrity paths untested. Phase 5 ships TWO verify-current-failure tests pinning today's state (rebuild raises `DataJointError` checksum; `_rebuild_nwb_artifact` itself runs + calls `_hash_nwb_recording`). Happy-path, hash-mismatch warning, and row-preservation tests deferred to main-epic recompute because the rebuild is not byte-deterministic vs the external-store checksum today (see overview decision 3 / C2 deferral). | 5 (deferred A/B/C → main-epic Phase 2) |
...
- Phase 5: A17's chunked path produces output bit-identical to the in-memory path on the smoke fixture (the equivalence test is the gating evidence); peak memory measured on a real Frank-lab session and documented in the PR. A18 ships TWO verify-current-failure tests (rebuild raises `DataJointError` checksum; `_rebuild_nwb_artifact` itself runs and calls `_hash_nwb_recording`); the happy-path / hash-mismatch / row-preservation tests defer to main-epic recompute per overview decision 3. A21 introduces the snapshot — first run pins the values; subsequent SI bumps surface as test failures.
