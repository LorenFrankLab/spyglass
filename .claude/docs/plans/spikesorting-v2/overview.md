...
1. **MountainSort 4 stays in v2.** Add `mountainsort4` to `SorterParameters` defaults alongside `mountainsort5`, `kilosort4`, `spykingcircus2`, `tridesclous2`, and the Spyglass special-case `clusterless_thresholder` peak-detection path. MS4 installs on Linux and the v1 sorter_params dicts can be ported with minimal changes (notably removing the `tempdir` field-mutation hack). Pydantic schema for MS4 ships in Phase 1's `_params/sorter.py`; non-Linux resolver/runtime gaps get platform-specific guards rather than removing MS4 globally.
