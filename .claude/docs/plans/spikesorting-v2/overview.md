...
| --- | --- | --- | --- | --- |
| `spikeinterface` | `>=0.99.1,<0.100` | `>=0.104,<0.105` | **Phase 0c prerequisite PR before Phase 1** | SortingAnalyzer, PreprocessingPipeline, modern curation primitives. Phase 0c ports v1 away from the WaveformExtractor back-compat shim to `create_sorting_analyzer` so v1 is native on the new pin. |
