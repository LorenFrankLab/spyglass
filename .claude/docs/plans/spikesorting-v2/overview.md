...
5. **Zero schema migration.** All v2 tables are designed in their final shape in Phase 1 (or in the phase that adds them). No `alter()` calls across phases. Concretely: Phase 1's `SortingSelection` declares both source parts (`RecordingSource`, `ConcatenatedRecordingSource`) with exactly-one-source enforcement in `insert_selection()` and `make()`; Phase 1's `Sorting.Unit`, `CurationV2.Unit`, and `CurationV2.UnitLabel` part tables are present from day one; Phase 1's `CurationV2` columns are final. The "two paths for `SortingSelection`" formerly in Open Question #8 is collapsed to one design.
