...
- **v0/v1 coexistence**: Legacy source and existing rows remain in-tree/queryable where possible throughout the v2 lifetime. Under the SI 0.104 environment, v2 is the supported runtime path for new processing; active v0/v1 runtime workflows require the legacy SI 0.99 environment unless Phase 0c explicitly ports a narrow compatibility shim. **No v1 source removal is planned in this work**.
...
Total v2 footprint: roughly 5300 LOC across the planned execution checkpoints. Compared to v1 at ~6000 LOC in `src/spyglass/spikesorting/v1/` (counted via `wc -l`), v2 is similar in size despite richer capability because the per-stage scaffolding is more consolidated.
