...
pipeline stage/behavior each exercises (recording, artifact_detection, sorting, curation, merge, concat,
...
const SPECIALIST_UNITS = [
  ...
  { key: 'type-design', isV2: true, agentType: 'pr-review-toolkit:type-design-analyzer', files: 'src/spyglass/spikesorting/v2/_params/*.py (Pydantic params models) and the DataJoint table definitions across v2', focus: 'Type/invariant design for Python Pydantic + DataJoint (translate compile-time / access-modifier language to Python idioms). For each Pydantic params model: are invariants enforced at construction (field types, validators, ranges, extra="forbid") so illegal states are unrepresentable? History: a permissive default (noise_levels=[1.0]) caused a 1400x divergence — missing/loose validation is the key risk class here. For DataJoint tables: do PK/FK definitions encode the intended cardinality and relationships (e.g. artifact_detection_id optionality, zero-unit gating via require_units) or are invariants left to runtime/external code? Rate encapsulation / expression / usefulness / enforcement and flag any invariant enforced only by documentation.' },
]
