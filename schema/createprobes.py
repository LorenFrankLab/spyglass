...
def add_probes():
    ...
    if {'probe_type' : 'tetrode'} not in device.Probe():
        ...
        device.Probe.insert1(probetype_dict, allow_direct_insert=True, replace=True)
        device.Probe.Shank.insert1(['tetrode', 0])
        for i in range(4):
            device.Probe.Electrode.insert1([['tetrode'], 0, i, 12.5, 0, 0, 0], replace=True)

        # add a 32 channel polymer probe type. FIX the x,y,z coordinates for each channel
    probetype_dict['probe_type'] = '32c-2s8mm6cm-20um-40um-dl' #check this name
    if {'probe_type': probetype_dict['probe_type']} not in device.Probe():
        ...
        for i in range(16):
            device.Probe.Electrode.insert1([['32c-2s8mm6cm-20um-40um-dl'], 0, i, 20, 0, 0, 0], replace=True)
        for i in range(16,32):
            device.Probe.Electrode.insert1([['32c-2s8mm6cm-20um-40um-dl'], 1, i, 20, 0, 0, 0], replace=True)
