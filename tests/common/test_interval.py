...
def test_plot_intervals(interval_list, start_time=0):
    fig = (interval_list).plot_intervals(
    ...
    fig_interval_list_names = np.array([label.get_text() for label in ax.get_yticklabels()])

    # extract interval list names from the IntervalList table
    fetch_interval_list_names = np.array(interval_list.fetch('interval_list_name'))

    # check that the interval list names match between the two methods
    assert array_equal(fig_interval_list_names, fetch_interval_list_names), "plot_intervals failed: plotted interval list names do not match"
    ...
    for collection in ax.collections:
        ...
        for patch in collection.get_paths():
            ...
            x_end = vertices[2, 0]
            collection_data.append([x_start * 60 + start_time, x_end * 60 + start_time])
    ...
    assert all_equal, "plot_intervals failed: plotted interval times do not match"
