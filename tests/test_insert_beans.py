...
def test_insert_sessions():
    print("In test_insert_sessions, os.environ['SPYGLASS_BASE_DIR'] is", os.environ['SPYGLASS_BASE_DIR'])
    ...
    try:
        ...
        if os.environ.get("KACHERY_CLOUD_EPHEMERAL", None) != "TRUE":
            print("Cannot load test file in non-ephemeral mode. Kachery cloud client may need to be registered.")
        raise e
