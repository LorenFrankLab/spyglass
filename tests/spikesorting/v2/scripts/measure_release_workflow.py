...
scratch-disk footprint (temp dir, analyzer cache root, analysis/recording
dirs). Writes ``<label>.json`` into ``--out-dir`` with stage timings, peak
memory, peak scratch, bundle bytes, the preflight effective configuration and
resource notes, the ``describe_run`` receipt and hardware details.
...
machine (they would share scratch and CPU), and note that on macOS
SpikeInterface spawns fresh worker processes per computation (slow imports),
so only Linux timings are representative of lab machines.
"""
...
class TreeMonitor:
    """Sample RSS of this process + all descendants, and scratch dir sizes."""

    def __init__(self, scratch_dirs, interval=0.25):
        self._proc = psutil.Process()
        self._scratch = [Path(d) for d in scratch_dirs]
...
def main():
    ...
    with TreeMonitor(scratch_dirs) as mon:
        ...
        sort_group_id = (
            args.sort_group_id
            if args.sort_group_id is not None
            else int(groups[0])
        )
        n_channels = len(
        ...
        if figpack_ok and result["n_units"] > 0:
        ...
        unit_ids = sorted(
            int(u)
            for u in (CurationV2.Unit & auto_ref.as_key()).fetch("unit_id")
        )
        if len(unit_ids) >= 2:
            ...
            if figpack_ok:
                with stage("review_bundle_after_merge"):
                    review2 = merged_ref.start_review(
                        args.profile, upload=False
                    )
                result["review_after_merge_bundle_bytes"] = (
                    TreeMonitor.dir_bytes(Path(review2.uri))
                )
        else:
            merged_ref = auto_ref
        ...
        with stage("select_units"):
            sel = select_units_for_analysis(
                merged_ref, policy="v2_unflagged_units"
            )
    ...
    result["peak_process_tree_rss_gb"] = round(mon.peak_rss / 1024**3, 3)
    result["peak_scratch_disk_gb"] = round(mon.peak_scratch / 1024**3, 3)
    result["monitor_samples"] = mon.samples
