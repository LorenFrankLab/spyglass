...
class LFPBandSelection(dj.Manual):
    ...
    def set_lfp_band_electrodes(
        ...
        filter_name : str
            The name of the filter (from the FirFilter schema)
        interval_name : str
            The name of the interval list (from the IntervalList schema)
        reference_electrode_list : list or int
            A single electrode ID corresponding to the reference to use for all electrodes or a list with one element per
            entry in the electrode_list
        ...
        # electrode_list
        query = LFPGroup.LFPElectrode() & {"nwb_file_name": nwb_file_name}
        ...
        LFPBandSelection.insert1(key, skip_duplicates=True)

        # get all of the current entries and delete any that are not in the list
        # elect_id, ref_id = (LFPBandSelection.LFPBandElectrode() & key).fetch(
        ...
        #         (LFPBandSelection.LFPBandElectrode() & key).delete()
        ...
        #     key["reference_elect_id"] = r
        #     LFPBandSelection.LFPBandElectrode().insert1(key, skip_duplicates=True)
