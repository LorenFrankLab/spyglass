...
try:
    ...
    def get_base_track_information(base_probabilities: xr.Dataset):
        ...
        x_width = round(
            (np.max(base_probabilities.x_position).item() - x_min)
            / (x_count - 1),
            6,
        )
        y_width = round(
            (np.max(base_probabilities.y_position).item() - y_min)
            / (y_count - 1),
            6,
        )
    ...
    def get_observations_per_frame(
        ...
        (times, time_counts_np) = np.unique(
            i_trim.time.values, return_counts=True
        )
