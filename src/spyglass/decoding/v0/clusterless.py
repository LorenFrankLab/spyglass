...
try:
    ...
    class UnitMarks(SpyglassMixin, dj.Computed):
        ...
        def _get_peak_amplitude(
            ...
            if estimate_peak_time:
                if peak_sign == "neg":
                    ...
                    peak_inds = np.argmax(
                        np.max(np.abs(waveform), axis=2), axis=1
                    )
        ...
        def _threshold(timestamps, marks, mark_param_dict):
            ...
            if mark_param_dict["peak_sign"] == "neg":
                include = (
                    np.min(marks, axis=1) <= -1 * mark_param_dict["threshold"]
                )
    ...
    class UnitMarksIndicator(SpyglassMixin, dj.Computed):
        ...
        def make(self, key):
            ...
            time = self.get_time_bins_from_interval(
        ...
        def get_time_bins_from_interval(interval_times, sampling_rate):
            ...
            n_samples = (
                int(np.ceil((end_time - start_time) * sampling_rate)) + 1
            )
        ...
        def fetch_xarray(self):
            ...
            return marks_indicators.assign_coords(
                {"marks": new_mark_names}
            ).sortby(["electrodes", "marks"])
    ...
    def populate_mark_indicators(
        ...
        CuratedSpikeSortingSelection().insert(
            curation_keys, skip_duplicates=True
        )
        ...
        for interval_name in tqdm(position_interval_names):
            ...
            marks_selection = (
                UnitMarks & mark_parameters_keys
            ) * position_interval
            marks_selection = (
