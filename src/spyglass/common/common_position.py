...
class IntervalPositionInfo(dj.Computed):
    ...
    def calculate_position_info_from_spatial_series(
        ...
        moving_average_window = int(
            position_smoothing_duration * sampling_rate)
        ...
        if is_upsampled:
            ...
            new_index = pd.Index(np.unique(np.concatenate(
                (position_df.index, new_time))), name='time')
        ...
        head_orientation[~is_nan] = gaussian_smooth(
            head_orientation[~is_nan], head_orient_smoothing_std_dev,
        ...
        velocity = get_velocity(
            ...
            time=time,
            sigma=speed_smoothing_std_dev,
            sampling_frequency=sampling_rate)  # cm/s
...
class TrackGraph(dj.Manual):
    ...
    def plot_track_graph_as_1D(self,
...
class PositionVideo(dj.Computed):
    ...
    def make(self, key):
        ...
        position_info_df = (IntervalPositionInfo() & {
            'nwb_file_name': key['nwb_file_name'],
            'interval_list_name': key['interval_list_name'],
            'position_info_param_name': key['position_info_param_name'],
        }
        ...
        output_video_filename = (f'{nwb_base_filename}_{epoch:02d}_'
        ...
        self.make_video(video_filename, centroids, head_position_mean,
                        ...
                        cm_to_pixels=cm_per_pixel, disable_progressbar=False)
    ...
    def make_video(self, video_filename, centroids, head_position_mean,
                   head_orientation_mean, video_time, position_time,
                   output_video_filename='output.mp4',
                   cm_to_pixels=1.0, disable_progressbar=False,
                   arrow_radius=15, circle_radius=8):
        RGB_PINK = (234, 82, 111)
        ...
        for time_ind in tqdm(range(n_frames - 1), desc='frames',
            ...
            if is_grabbed:
                ...
                if np.all(~np.isnan(head_position)) & np.all(~np.isnan(head_orientation)):
                    arrow_tip = (
                        int(head_position[0] + arrow_radius *
                            np.cos(head_orientation)),
                        int(head_position[1] + arrow_radius *
    ...
    def __init__(self, ax, video_filename, alpha_other=0.3):
        ...
        self.poly = PolygonSelector(
            ...
            markerprops=dict(marker='o', markersize=7,
                             mec='white', mfc='white', alpha=0.5, zorder=10),
        )
        self.ind = []
        self.video_filename = video_filename
        self.video = cv2.VideoCapture(video_filename)
        ...
        x, y = np.meshgrid(np.arange(self.frame.shape[1]),
                           np.arange(self.frame.shape[0]))
