...
class AutomaticCuration(dj.Computed):
    ...
    def make(self, key):
        ...
        merge_groups, units_merged = self.get_merge_groups(parent_sorting, parent_merge_groups, quality_metrics, merge_params) 
        if units_merged:
            # get merged sorting extractor here
            return NotImplementedError

        label_params = (AutomaticCurationParameters & key).fetch1('label_params')
        labels = self.get_labels(sorting, parent_labels, quality_metrics, label_params)
...
class FinalizedSpikeSorting(dj.Manual):
    ...
    def make(self, key):
        ...
        try:
            ...
            raise Exception(f'Metrics for Curation {key} must be calculated before it can be inserted here')
            return
